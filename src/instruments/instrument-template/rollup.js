...
module.exports = ({ name, outputDir }) => ({
  ...
  writeBundle(_config, bundle) {
    ...
    const process = (s) => {
      let tmp = s;
      tmp = replaceButSad(tmp, "INSTRUMENT_NAME", name.toUpperCase());
      tmp = replaceButSad(tmp, "INSTRUMENT_NAME_LOWER_SKEWER", name);
      tmp = replaceButSad(tmp, "INSTRUMENT_NAME_SKEWER", name);
      tmp = replaceButSad(
        tmp,
        "INSTRUMENT_NAME_LOWER",
        snakeCaseName.toLowerCase()
      );
      // tmp = replaceButSad(tmp, "INSTRUMENT_NAME", snakeCaseName);
